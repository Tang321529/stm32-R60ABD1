...
unsigned char setnum[5]={50,10,90,50,40};
...
void menu_1()
{
	...
	if(getnum[0] > setnum[0] || getnum[0] < setnum[1])
	{
		OLED_ShowString(4, 4, "B");
	}
	if(getnum[1] > setnum[2] || getnum[1] < setnum[3])
	{
		OLED_ShowString(4, 6, "H");
	}
	if(getnum[2] > setnum[4])
	{
...
void menu_3()
{
	...
	OLED_ShowNum(1,3,setnum[0],2);
	OLED_ShowString(1, 5, "-");
	OLED_ShowNum(1,6,setnum[1],2);
	
	OLED_ShowNum(2,3,setnum[2],2);
	OLED_ShowString(2, 5, "-");
	OLED_ShowNum(2,6,setnum[3],2);
	
	OLED_ShowNum(3,3,setnum[4],2);
...
void num_set()
{
	if(KeyNum==2)
	{
		...
		if(select_number==5){select_number=0;}
